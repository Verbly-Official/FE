...
import { getSubscriptionStatusApi, type SubscriptionDto } from '../../../apis/payment';
...
const Banner: React.FC = () => {
  ...
  const handlePaymentClick = () => navigate('/my/korean/payment');
  const handleManageClick = () => navigate('/my/korean/payment');
  ...
  if (isPremium) {
    ...
    return (
      ...
      >
        ...
        <img src={Banner2Icon} alt="Premium User" className="hidden md:block absolute left-[-10px] bottom-[-10px] w-[140px] lg:w-[180px] h-auto z-10 pointer-events-none object-contain drop-shadow-md" />

        <div className="relative z-10 w-full flex justify-between items-center px-6 md:pl-[160px] lg:pl-[200px] py-4">
          <div className="flex flex-col gap-1">
            <p className="text-[length:var(--fs-title1)] font-bold text-white leading-tight">
              Premium Pro 이용 중
            </p>
            <p className="text-[length:var(--fs-body1)] text-violet-100 font-medium">
              {startDate}부터 모든 혜택을 누리고 있어요!
            </p>
          </div>
          <img src={ChevIcon} alt="Go" className="w-6 h-6 md:w-8 md:h-8 opacity-40 shrink-0" />
        </div>
      </div>
    );
  }
